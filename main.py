...
# ============================================================
# SỰ KIỆN KHI BOT VÀO SERVER MỚI
# ============================================================
@bot.event
async def on_guild_join(guild):
    try:
        # 1. Tìm người mời bot qua audit log
        inviter = None
        try:
            ...
            log(f"⚠️ Không có quyền xem audit log trong {guild.name}", "WARN")
        except Exception as e:
            log(f"⚠️ Lỗi khi truy xuất audit log: {e}", "WARN")

        if inviter is None:
            inviter = guild.owner
            if inviter is None:
                inviter = bot.user
                log(f"⚠️ Không tìm thấy người mời và không có owner, sử dụng bot user làm đại diện", "WARN")
            else:
                log(f"ℹ️ Không tìm thấy người mời bot, sử dụng owner: {inviter}", "WARN")

        # 2. Tạo category "LUNAL" nếu chưa có (với retry)
        category = discord.utils.get(guild.categories, name="LUNAL")
        if not category:
            for attempt in range(3):
                try:
                    category = await guild.create_category("LUNAL")
                    log(f"✅ Đã tạo category LUNAL trong {guild.name}", "INFO")
                    break
                except discord.HTTPException as e:
                    if e.status == 429:
                        retry_after = float(e.response.headers.get("Retry-After", 1))
                        log(f"⏳ Rate limit khi tạo category, chờ {retry_after}s (lần {attempt+1}/3)", "WARN")
                        await asyncio.sleep(retry_after)
                    else:
                        log(f"❌ Không thể tạo category: {e}", "ERROR")
                        break
                except Exception as e:
                    log(f"❌ Lỗi không xác định khi tạo category: {e}", "ERROR")
                    break
            else:
                log(f"❌ Không thể tạo category sau 3 lần thử", "ERROR")
                category = None

        # 3. Tạo kênh riêng cho người mời (chỉ khi inviter không phải bot user)
        channel = None
        if inviter.id != bot.user.id:
            channel_name = f"lunal-{inviter.name.lower().replace(' ', '-')}"
            overwrites = {
                guild.default_role: discord.PermissionOverwrite(view_channel=False),
                inviter: discord.PermissionOverwrite(
            ...
            try:
                ...
                log(f"✅ Đã tạo kênh riêng {channel_name} cho {inviter}", "INFO")
                await channel.send(f"👋 Xin chào {inviter.mention}! Đây là kênh riêng của bạn. Bot sẽ tiến hành kick tất cả bot khác trong server. Hãy xem kết quả bên dưới.")
            except discord.Forbidden:
                log(f"❌ Bot không có quyền tạo kênh trong {guild.name} (thiếu manage_channels)", "ERROR")
            except discord.HTTPException as e:
                if e.status == 429:
                    log(f"⏳ Rate limit khi tạo kênh, bỏ qua", "WARN")
                else:
                    log(f"❌ Lỗi HTTP khi tạo kênh: {e}", "ERROR")
            except Exception as e:
                log(f"❌ Lỗi không xác định khi tạo kênh: {e}", "ERROR")
                channel = None
        else:
            log("ℹ️ Không tạo kênh vì người mời là bot.", "WARN")

        # 4. Thực hiện kick bot (có kiểm tra quyền)
        log(f"🚀 Bắt đầu kick bot trong server {guild.name} ({guild.id})", "INFO")
        if not guild.me.guild_permissions.kick_members:
            log(f"❌ Bot không có quyền kick thành viên trong {guild.name}", "ERROR")
            kicked = 0
            failed = 0
        else:
            kicked, failed = await kick_all_bots(guild)

        result_msg = f"**Kết quả kick bot trong {guild.name}:**\n- Đã kick: {kicked} bot\n- Thất bại: {failed} bot"
        log(result_msg, "INFO")

        # 5. Gửi kết quả vào kênh riêng (nếu có)
        if channel:
            try:
                await channel.send(result_msg)
            except discord.NotFound:
                log(f"❌ Kênh riêng không tồn tại hoặc đã bị xóa", "ERROR")
            except Exception as e:
                log(f"❌ Không thể gửi kết quả vào kênh riêng: {e}", "ERROR")

        # 6. Gửi báo cáo về kênh log (LOG_CHANNEL_ID)
        embed = discord.Embed(
            title="🤖 Auto Kick Bot on Join",
            description=f"Server: **{guild.name}** (ID: {guild.id})",
            color=0x00FF88 if kicked > 0 else 0xFFAA00,
            timestamp=utcnow()
        )
        embed.add_field(name="👤 Người mời", value=inviter.mention if inviter else "Không xác định", inline=True)
        embed.add_field(name="📢 Kênh riêng", value=channel.mention if channel else "❌ Không tạo được", inline=True)
        embed.add_field(name="✅ Bot đã kick", value=str(kicked), inline=True)
        embed.add_field(name="❌ Bot thất bại", value=str(failed), inline=True)
        ...
        if log_channel:
            try:
                ...
                log(f"❌ Không thể gửi embed đến log channel: {e}", "ERROR")
        else:
            log(f"⚠️ LOG_CHANNEL_ID ({LOG_CHANNEL_ID}) không tồn tại hoặc bot không có quyền.", "WARN")

    except Exception as e:
        log(f"💥 Lỗi tổng thể trong on_guild_join: {e}", "ERROR")
